...
describe("ParkingStore", function () {
  ...
  it("consumption electric", async () => {
    ...
    await parkingStoreContract.getElectric(tokenId).then((b) => {
      expect(b).to.be.equal(100);
    });

    await expect(parkingStoreContract.connect(addr1).consumption(tokenId, 50))
      .to.be.reverted;
    await parkingStoreContract
      .connect(owner)
      .grantRole(ethers.utils.id("GAME_ROLE"), addr1.address);
    await expect(
      parkingStoreContract.connect(addr1).consumption(tokenId, 50)
    ).to.be.emit(parkingStoreContract, "ElectricChange");
    await parkingStoreContract.getElectric(tokenId).then((b) => {
      expect(b).to.be.equal(50);
    });
    // Overconsumption should be 0
    await expect(
      parkingStoreContract.connect(addr1).consumption(tokenId, 70)
    ).to.be.emit(parkingStoreContract, "ElectricChange");
    await parkingStoreContract.getElectric(tokenId).then((b) => {
  ...
  it("charge car", async () => {
    ...
    await expect(
      parkingStoreContract.connect(addr1).charge(tokenId, 20)
    ).to.be.emit(parkingStoreContract, "ElectricChange");

    await parkingStoreContract.getElectric(tokenId).then((b) => {
      expect(b).to.be.equal(70);
    });
    await parkingStoreContract.connect(addr1).charge(tokenId, 90);
    await parkingStoreContract.getElectric(tokenId).then((b) => {
      expect(b).to.be.equal(100);
    });
  });
